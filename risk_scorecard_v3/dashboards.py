...
from components.components.dropdown_menu import DropdownMenu
from risk_scorecard_v3.data import (
    get_business_supported_data,
    get_wave_data,
    get_tower_name_dropdown,
    BaseScoresByVPSerializer,
    BaseOpenVulnerabilitiesSerializer,
    BaseControlFindingsSerializer,
    BaseBAPPInformationSerializer,
    get_financial_data,
)


def get_filter_options(
    tower_name: str, wave_ids: list = None, financial_str: str = None
):
    filter_options = f" where tower_name = '{tower_name}'"

    if wave_ids is not None and "All" not in wave_ids:
        result = ",".join(f"'{num.strip()}'" for num in wave_ids)
        filter_options += f" and wave_id in ({result})"

    if financial_str is not None and "All" not in financial_str:
...
class TowerHomeDashboard(Dashboard):
    def __init__(
        ...
        **kwargs,
        ...
        tower_dropdown = get_tower_name_dropdown(tower_name)
        wave_dropdown = get_wave_data(wave_ids)
        financial_dropdown = get_financial_data(financial_str)

        self.components["dropdown_menu"] = DropdownMenu(
            grid_css_classes="span-12",
            value=[tower_dropdown, wave_dropdown, financial_dropdown],
        )
        self.components["gauge"] = get_business_supported_data(filter_options)

        self.components["scores_by_vp_table_title"] = Text(
            value="Scores by VP", grid_css_classes="span-12", css_classes="card-title"
        )
        ...
        ScoresByVPSerializer = BaseScoresByVPSerializer.set_filter_options(
            filter_options
        )

        self.components["scores_by_vp_table"] = DataTable(
            value=ScoresByVPSerializer, grid_css_classes="span-12"
        )
        ...
        BAPPInformationSerializer = type(
            "BAPPInformationSerializer",
            (BaseBAPPInformationSerializer,),
            class_attributes,
        )
        ...
        self.components["bapp_information_table"] = DataTable(
            value=BAPPInformationSerializer, grid_css_classes="span-12"
        )
        ...
        ControlFindingsSerializer = type(
            "ControlFindingsSerializer",
            (BaseControlFindingsSerializer,),
            class_attributes,
        )
        ...
        self.components["control_findings_table"] = DataTable(
            value=ControlFindingsSerializer, grid_css_classes="span-12"
        )
        ...
        OpenVulnerabilitiesSerializer = type(
            "OpenVulnerabilitiesSerializer",
            (BaseOpenVulnerabilitiesSerializer,),
            class_attributes,
        )
        ...
        self.components["open_vuln_table"] = DataTable(
            value=OpenVulnerabilitiesSerializer, grid_css_classes="span-12"
        )
